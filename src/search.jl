...
function get_Q_id(Q::Vector{Node{State}}, t::Int64, next::Int64)::String where State<:AbsState
...
function search!(
    ...
    roadmaps::Vector{Vector{Node{State}}} = (
        map(e -> [Node{State}(e[1], 1, []), Node{State}(e[2], 2, [])], zip(config_init, config_goal))
    ),
    NUM_REFINE_AGENTS::Int64 = 1,
    AGENTS_REFINE::Vector{Int64} = (
        current_solution == nothing
        ? collect(1:length(config_init))  # initial solution
    ...
    if VERBOSE > 0 && is_refine; println("\trefined agents:", AGENTS_REFINE); end
    ...
    for k = 1:MAX_ITER
        ...
        while !isempty(OPEN) && !timeover()
            ...
            if S.next > 0
                ...
                j = (
                    S.next < length(AGENTS_REFINE)
                    ? S.next + 1
                    : (is_refine ? 0 : 1)  # refinement or initial solution
                )
                ...
                Q_new = copy(S.Q)
                t = min(S.t+1, T)
                for k in AGENTS_FIXED; Q_new[k] = current_solution[t][k]; end
