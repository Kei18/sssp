...
to_string(s::StateArm2) =
    @sprintf("(x: %.4f, y: %.4f, theta1: %.4f, theta2: %.4f)", s.x, s.y, s.theta1, s.theta2)
...
function gen_connect(
    ...
    return (q_from::StateArm2, q_to::StateArm2, i::Int64; ignore_eps::Bool = false) -> begin
        # avoid far points
        ...
        for e in (0:step) / step
            ...
            if !all(0 <= z <= 1 for z in vcat(p1, p2))
                return false
            end

            # check: collisions with obstacles
            if any([dist(p0, p1, [o.x, o.y]) < o.r for o in obstacles])
                return false
            end
            if any([dist(p1, p2, [o.x, o.y]) < o.r for o in obstacles])
                return false
            end
        end
...
function gen_collide(q::StateArm2, rads::Vector{Float64}; step::Int64 = 10)::Function
    @gen_collide(
        ...
        begin
            ...
            if segments_intersect(p_i_3, p_i_4, p_j_0, p_j_1)
                return true
            end
            if segments_intersect(p_i_3, p_i_4, p_j_0, p_j_2)
                return true
            end
            if segments_intersect(p_i_3, p_i_4, p_j_1, p_j_3)
                return true
            end
            if segments_intersect(p_i_3, p_i_4, p_j_2, p_j_4)
                return true
            end

            if segments_intersect(p_j_3, p_j_4, p_i_0, p_i_1)
                return true
            end
            if segments_intersect(p_j_3, p_j_4, p_i_0, p_i_2)
                return true
            end
            if segments_intersect(p_j_3, p_j_4, p_i_1, p_i_3)
                return true
            end
            if segments_intersect(p_j_3, p_j_4, p_i_2, p_i_4)
                return true
            end
            ...
            for e_i in (0:step) / step, e_j in (0:step) / step
                ...
                # collision check
                if segments_intersect(p_i_0, p_i_1, p_j_0, p_j_1)
                    return true
                end
                if segments_intersect(p_i_0, p_i_1, p_j_1, p_j_2)
                    return true
                end
                if segments_intersect(p_i_1, p_i_2, p_j_0, p_j_1)
                    return true
                end
                if segments_intersect(p_i_1, p_i_2, p_j_1, p_j_2)
                    return true
                end
            end
            return false
